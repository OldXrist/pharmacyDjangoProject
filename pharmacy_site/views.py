    ...
    if request.method == 'POST':
        ...
        if form.is_valid():
            ...
            redirect_url = reverse(
                'prescription-calculate') + f'?id={prescription.id}'
...
def prescription_calc(request):
    ...
    def ocr_core(filename):
        text = pytesseract.image_to_string(
            Image.open(filename), lang='eng', config='--psm 6')
        text.strip()
        return text.strip()

    # change 'replace()' if ur using linux or mac
    filename = os.path.abspath(os.path.join(
        os.getcwd(), '.')+image_url.replace('/', '\\'))
    ...
    if request.method == 'POST':
        ...
        for product in products:
            ...
            OrderDetail.objects.create(
                order=order, medicine=product, quantity=quantity, price=price)
